#!/bin/bash
#
# APK Bug Bounty Skill Installer
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --path)
            ...
            ;;
        --help|-h)
            echo "APK Bug Bounty Skill Installer"
            echo ""
...
echo -e "${BLUE}╔════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     APK Bug Bounty Skill Installer             ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════╝${NC}"
echo ""
...
REQUIRED_FILES=(
    "SKILL.md"
    "references/android-impact-matrix.md"
)
...
if $ALL_OK; then
    ...
    echo "  Ask Claude to analyze a decompiled APK for security vulnerabilities"
    echo "  Example: 'Analyze this APK for bug bounty vulnerabilities'"
    echo ""
else
