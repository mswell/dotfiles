...
const EXTRACTION_PROVIDER = "github-copilot";
const GPT_MODEL_ID = "gpt-5.3-codex";
const HAIKU_MODEL_ID = "claude-haiku-4.5";

/**
 * Prefer GPT (copilot) for extraction when available, otherwise fallback to haiku (copilot) or the current model.
 */
async function selectExtractionModel(
	currentModel: Model<Api>,
	modelRegistry: ModelRegistry,
): Promise<Model<Api>> {
	const gptModel = modelRegistry.find(EXTRACTION_PROVIDER, GPT_MODEL_ID);
	if (gptModel) {
		const auth = await modelRegistry.getApiKeyAndHeaders(gptModel);
		if (auth.ok) {
			return gptModel;
		}
	}

	const haikuModel = modelRegistry.find(EXTRACTION_PROVIDER, HAIKU_MODEL_ID);
	if (!haikuModel) {
		return currentModel;
	}
